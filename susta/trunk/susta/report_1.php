...
$pippo = $dati['anagrafica'];

if ( false )
{
	$pippo = $dati['anagrafica'];
	
	//$dati['anagrafica']= array();
	foreach ( $pippo as $rec )
	{
		$dati['anagrafica'][] = $rec;
	//	$dati['anagrafica'][0] = $rec;
	}
	/**/
	//evid( $dati );
}
...
<?php 
	$i = 0;
	$rowPerPag = 31;
	$totPag = floor( count( $dati['anagrafica'] ) / $rowPerPag ) + 1; 
	$pNum = 1;
	
	while ( $i < count( $dati['anagrafica'] ) ) {  ?>
...
<thead>
	...
	<td class="intestazione">
		<?php //printIntestazione("ANAGRAFICA", "&nbsp;", "&nbsp;", $pNum++, $totPag); ?>
	</td>
	</tr>
</thead>
<tbody>
    <tr><!-- DATI ANAGRAFICI -->
       <td>
           ...
           <table id="anagrafica" cellpadding="0" cellspacing="0" border="0" width="100%" class="stono" style="overflow: auto;">
...
<?php 	} else { 
?>
                  <tr><td><?php echo $riga['cognome']; ?></td><td><?php echo $riga['nome']; ?></td></tr>
<?php
		}
//		if ( !$riga ) break;
		$i++;  
...
<![endif]-->	
        ...
        <tr>
          <td rowspan="3" class="sx" width="33%">STIVAL s.r.l.</td>
          <td rowspan="3" class="centro" width="33%"><?php echo $titolo; ?></td>
          <td class="dx1" width="33%"><?php echo $modulo; ?></td>
        </tr>
        <tr>
